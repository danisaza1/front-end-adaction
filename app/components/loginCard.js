...
export default function LoginCard() {
  ...
  onChange={(e) => setEmail(e.target.value)}/>
       ...
       <button id="login-button" type="submit" className="mt-5 gap-2 flex justify-center bg-emerald-600 shadow-xl hover:bg-emerald-500 text-white font-bold border rounded-xl shadow p-3 w-full"> <LogIn /> Se connecter</button>
        <button id="register-button" type="button" className="mt-5 gap-2 flex justify-center bg-emerald-600 shadow-xl hover:bg-emerald-500 text-white font-bold border rounded-xl shadow p-3 w-full" onClick={() => router.push('/admin/volunteers')}>  <Users /> Gérer les bénévoles</button> 
        </div>
      </form>
    </div>
  );
