...
export default function Formulaire({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (volunteerToEdit) {
        res = await fetch(
          `http://localhost:3001/updateProfil/${volunteerToEdit.id}`,
          {
            method: "PATCH",
      ...
      if (res.ok) {
        ...
        console.error(
          "Failed to submit volunteer data:",
          res.status,
          res.statusText,
          errorData
        );
        alert(
          `Erreur: ${
            errorData.error ||
            "Impossible de soumettre les données du bénévole."
          }`
        );
      }
    } catch (error) {
      ...
      alert(
        "Une erreur réseau s'est produite lors de la soumission du formulaire."
      );
    }
  };
        ...
        <div>
          ...
          {!volunteerToEdit && (
            <p className="text-sm text-gray-500 mt-1">
              Le mot de passe est requis pour un nouveau bénévole.
            </p>
          )}
          {volunteerToEdit && (
            <p className="text-sm text-gray-500 mt-1">
