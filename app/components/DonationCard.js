...
export default function DonationCard({
  ...
  return (
    <Card className="flex flex-col items-start justify-between border border-gray-100 rounded-lg p-3 transition-all duration-200 ease-in transform hover:-translate-y-1 hover:scale-102">
      <h3 className="flex text-lg font-bold text-gray-800 h-10">
        {" "}
        <span className="text-2xl mr-3 inline-block -mt-1.5">{emoji}</span>
        {assoName}
      </h3>
      ...
      <div className="flex flex-row w-full">
        ...
        <Button
          className="bg-emerald-600 hover:bg-emerald-700 flex flex-auto"
          variant="default"
          size="default"
          onClick={() => fetchD(assoName, parseInt(points))}
        >
          Faire un don
        </Button>
      </div>
