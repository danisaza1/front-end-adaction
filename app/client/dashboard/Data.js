...
export default function UseWastesData(selectedDate) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (!user?.token) {
        console.log(
          "UseWastesData: No user token found, skipping dashboard data fetch."
        );
        setWastesData([]);
      ...
      if (
        ...
        console.warn(
          "UseWastesData: Invalid selectedDate provided. Skipping data fetch."
        );
        setWastesData([]);
      ...
      console.log(
        `UseWastesData: Fetching data for Month: ${month}, Year: ${year}`
      );

      try {
        const response = await fetch(
          `http://localhost:3001/dashboard?month=${month}&year=${year}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          ...
          console.error(
            "UseWastesData: Dashboard API Error Response:",
            errorJson
          );
          throw new Error(
            `Erreur du serveur: ${response.status} - ${
              errorJson.error || "Erreur inconnue"
            }`
          );
        }
        ...
        if (json && Array.isArray(json.wastes)) {
          ...
          console.warn(
            "UseWastesData: Unexpected dashboard data format, expected { wastes: [], firstname: '' }:",
            json
          );
          setWastesData([]);
